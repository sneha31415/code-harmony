...
export const ContentUpload = ({ onFileUpload }) => {
  ...
  console.log('Token retrieved:', token); // Debugging
    ...
    formData.append('instrumentUsed', selectedInstrument);

    try {
      setIsLoading(true);
      const response = await axios.post('http://localhost:5000/api/audio/process', formData, {
        headers: {
      ...
      setGeneratedTrack({
        name: file.name,
        url: `http://localhost:5000/${conversion.processedFilePath}`,
      });
      console.log('Generated track:', conversion.processedFilePath); // Debugging
      setError(null);
    } catch (err) {
  ...
  return (
    <div className="container">
      <div className="generate-section">
        ...
        <div className="generated-preview">
          <div className="generated-track">
            {generatedTrack ? (
              <AudioPlayer audioUrl={generatedTrack.url} />
            ) : (
      ...
      <div className="previous-creation" style={{ background: '#1e1e1e' }}>
        <PreviousCreation files={files} />
      </div>
    </div>
  );
};